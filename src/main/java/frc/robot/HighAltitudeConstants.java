...
public final class HighAltitudeConstants {
    ...
    //assisted mode with the dragonfly. The higher the constant, the sharper the correction.
    public static double DRIVETRAIN_DRAGONFLY_TURN_CORRECTION = 0.01;
    
    //The sides of the robot normally move faster than the dragonfly module, this constant 
    //Indicates the rate. 0.60 means that the dragonfly module has 60% of the speed of the sides.
    ...
    //When turning in autonomous at power 1, if the difference between the target angle and the 
    //current angle (in degrees) is less than this constant, it will start braking. Note that 
    //this constant is proportional to the square of the turning speed.  
    public static double DRIVETRAIN_AUTO_TURNING_BRAKING_DISTANCE = 100;
    //When turning in autonomous, if the difference between the target angle and the current angle
    //(in degrees) is less than this constant, it be consider on target.
    public static double DRIVETRAIN_AUTO_TURNING_ARRIVE_OFFSET = 3;

    ///Straight motion

    //When moving straight (straightMove()) in autonomous at power 1, if the difference between 
    //the target and the current position (in meters) is less than this constant, it will start 
    //braking. Note that this constant is proportional to the square of the speed (from -1 to 1).  
    public static double DRIVETRAIN_AUTO_STRAIGHT_BRAKING_DISTANCE = 1;
    //When moving straight (straightMove()) in autonomous, if the difference between the target 
    ...
    //When moving straight while keeping an angle (straightMove()), this constant will determine 
    ...
    //The reported encoder position per revolution, check encoder specifications. 
    ...
    // NEVER, ABSOLUTELY NEVER APPROXIMATE THIS, USE ONLY FRACTIONS WITH WHOLE NUMBERS.
    ...
    //Use these constants to convert from encoder position to meters (position*these constants = meters)
