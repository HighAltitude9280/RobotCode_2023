...
public class SplineGenerator {
     ...
     * 
     * @param controlPoints 2-row matrix of the control points, where the first
     *                      column contains
     *                      the x coordinates of the control points and the second
     *                      the y coordinates.
     * 
    ...
    public static CubicSpline generateNaturalSpline(double[][] controlPoints) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            // Each polynomial has two equations, f(x_i)=y_i and f(x_i+1)=y_i+1, therefore,
            // as seen in the pdf document that explains the construction of this class
            // The row 2i and 2i+1 will be used for that matter.

            // The 4i+k represents the kth coefficient of the ith polynomial. For example, i
            // = 2 and k = 0 represents a_i, that is to say, the cubic coefficient of the
            // ith polynomial.
        ...
        // Second constraint f_k'(x)=f_k'(x) for all control points except first and
        // last
        // The first empty row is the row number 2n-2, all the others have been occupied
        ...
        // Third constraint f_k''(x)=f_k''(x) for all control points except first and
        // last
        // The first empty row is the row number 3n-4, all the others have been occupied
        // by the first constraint
        nextEmptyRow = 3 * n - 4;
        ...
        // Since it's a natural spline, the two final constraints will be f_0''(x_0)=0
        // and f_n-1''(x_n)=0
    ...
    /**
     * Generates a hermite clamped spline that interpolates between the control
     * points and its
     * derivative at the first and last point is as specified.
     * 
     * @param controlPoints     2-row matrix of the control points, where the first
     *                          column contains
     *                          the x coordinates of the control points and the
     ...
     * @return Piece-wise spline that interpolates between the control points.
     */
    public static CubicSpline generateHermiteClampedSpline(double[][] controlPoints, double initialDerivative,
        ...
        // Since it's a hermite clamped spline, the two final constraints will be
        // f_0'(x_0)=initialDerivative and f_n-1'(x_n)=finalDerivative
    ...
    /**
     * This class treats the first column of the control matrix as the independent
     * variable and the
     * second as the dependent variable. Normally, the first column represents x
     * values and the second
     * y values. Therefore, for a vertical spline, that is, when y is the
     * independent variable, axis
     * must be swapped. This method switches the first and second column of a
     * matrix. Note that this
